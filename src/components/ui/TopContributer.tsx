...
export default async function TopContributer() {
  const topContributer = await users.list<UserPrefs>([
    Query.limit(10),
  ]);
  ...
  return (
    <div className="relative flex max-h-[400px] min-h-[400px] w-full max-w-[32rem] flex-col overflow-hidden rounded-lg bg-white/10 p-6 shadow-lg" >
      <AnimatedList>{topContributer.users.map((user) => <Notification key={user.$id} user={user} />)}</AnimatedList>
    </div>
  )
