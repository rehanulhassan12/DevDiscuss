...
function Answer({
  ...
  const [answer, setAnswer] = React.useState(answers)
  ...
  const handleAnswer = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (!newAnswer || !user) {
      window.alert("Please login to answer this question");
      return
    }
    ...
    try {
      ...
      setAnswer(prev => ({ ...prev, total: prev.total + 1, documents: [{ ...data, author: user, upvotesDocuments: { documents: [], total: 0 }, downvotesDocuments: { documents: [], total: 0 }, comments: { documents: [], total: 0 } }, ...prev.documents] }))


    } catch (error: any) {
      console.log("error in adding answer", error);
      window.alert(error?.message || "error in adding answer");
    }
  }
  ...
  const deleteAnswer = async (answerId: string) => {
    try {
      ...
      if (!response.ok) return
      const data = await response.json()
      setAnswer(prev => ({ ...prev, total: prev.total - 1, documents: prev.documents.filter((answer) => answer.$id !== answerId) }))

    } catch (error: any) {
      console.log("error in deleting answer", error);
      window.alert(error?.message || "error in deleting answer");
